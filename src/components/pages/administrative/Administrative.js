...
class Administrative extends Component {
  ...
  state = {
    administrative:
      this.props.intl.locale === "en" ? administrativeEN : administrativeBG,
    administrativeProjects:
      this.props.intl.locale === "en"
        ? Object.values(administrativeProjectsEN)
        : Object.values(administrativeProjectsBG),
  ...
  updateActiveItem = () => {
    if (
      ...
      this.props?.location.hash !== this.state.openAccordionItem
    ) {
      this.setState({
        openAccordionItem: this.props.location.hash,
        openAccordionItemPured: Number(
          this.pureTheItem(this.props.location.hash)
        ),
      });
  ...
  getProjectContent = (projectId, articleId) => {
    let project = this.state.administrativeProjects.find(
    ...
    return (
      <div className="administrative-page">
        {this.state.administrative.map((doc) => {
          return (
            <div key={doc.id} className="administrative-block">
              <HeroInner title={doc.type} />
              <AccordionBlock
  ...
  renderDetailAdministrativePage = () => {
    let isProjectExist = this.isProjectExist(
      Number(this.props.match.params.parentId),
