...
export function isIOS(): boolean {
    let iOS = /iPad|iPhone|iPod/.test(navigator.userAgent) && !(window as any).MSStream
    return iOS
}
...
export function indexOfCollection(collections: any, type: string) {
    console.log(collections.findIndex((el: { collectionsType: string }) => el.collectionsType === type))
    return collections.findIndex((el: { collectionsType: string }) => el.collectionsType === type)
}

export function setCollectionFromMain(collections: any, type: string) {
    return collections.find((el: { collectionsType: string }) => el.collectionsType === type)
}
